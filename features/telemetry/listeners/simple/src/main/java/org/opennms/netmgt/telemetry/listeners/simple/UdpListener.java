...
public class UdpListener implements Listener {
                        ...
                        ch.pipeline()
                                .addLast(new SimpleChannelInboundHandler<DatagramPacket>() {
                                    ...
                                    protected void channelRead0(final ChannelHandlerContext ctx,
                                        ...
                                        UdpListener.this.parser.parse(msg.content().nioBuffer(), msg.sender(), msg.recipient());
                                    }
                                })
    ...
    public void setParsers(final Set<Parser> parsers) {
        ...
        final Parser parser = parsers.iterator().next();
        if (!(parser instanceof UdpParser)) {
            throw new IllegalArgumentException("Parser must be a UDP parser");
        }

        this.parser = (UdpParser) parser;
    }
